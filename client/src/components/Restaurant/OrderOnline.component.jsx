  ...
  useEffect(() => {
    restaurant &&
      dispatch(getFoodList(restaurant.menu)).then((data) => {
        setMenu(data.payload.menus);
        setSelected(data.payload.menus[0].name);
      });
  }, [restaurant]);
  ...
  return (
    <>
      <div className="w-full flex items-start">
        <aside className="sticky top-64 hidden md:flex flex-col gap-1 border-r overflow-y-scroll border-gray-200 w-1/4 h-screen py-4">
          {menu?.map((item, index) => (
            <MenuListContainer
              {...item}
              key={index}
